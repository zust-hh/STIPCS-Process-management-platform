...
$sql="SELECT * FROM project WHERE isOpen = '1' ORDER BY `project`.`begin_time` DESC LIMIT ".(($page-1)*5).",5";
...
echo " <div class=\"manage-content-b-block\">
    ...
    if ($row["position"]==0) {
                    	echo "<div class=\"block-content-t\"><p>".$row["title"]."</p><span style=\"font-family=\"微软雅黑\"\">正在进行</span></div>";
                    }
     else if ($row["position"]==1) {
                          echo "<div class=\"block-content-t\"><p>".$row["title"]."</p><span style=\"font-family=\"微软雅黑\"\">已完成</span></div>";
                                    }  
          else
          				echo "<div class=\"block-content-t\"><p>".$row["title"]."</p><span style=\"font-family=\"微软雅黑\"\">未完成</span></div>";
...
$total_sql="SELECT COUNT(*) FROM project";
$total_result=mysqli_fetch_array(mysqli_query($conn,$total_sql));
$total=$total_result[0];//总条数
...
if ($page>1){
	$pagebanner.="<li><a href='javascript:void(0)' onclick='ajx1(1)'>首页</a></li>";
	$pagebanner.="<li><a href='javascript:void(0)' onclick='ajx1(".($page-1).")'>上一页</a></li>";
}
...
for($i=$start;$i<=$end;$i++)
	{
		$pagebanner.="<li><a href='javascript:void(0)' onclick='ajx1(".$i.")'>".$i."</a></li>";
	}
...
if ($page<$total_pages) {
	$pagebanner.="<li><a href='javascript:void(0)' onclick='ajx1(".($page+1).")'>下一页</a></li>";
	$pagebanner.="<li><a href='javascript:void(0)' onclick='ajx1(".$total_pages.")'>尾页</a></li>";
}
